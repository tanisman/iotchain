...
{
	logger(log_level::debug).format("recv_buffer_.size()={}, recv size={}", receive_buffer_.size(), size);
	...
	while (ptr != end)
	{
		...
		logger(log_level::debug).format("total_size={}, available_bytes={}, remaining_bytes={}", recv_total_size_, available_bytes, remaining_bytes);
		...
		if (receive_buffer_.size() == sizeof(message_header))
		{
			...
			logger(log_level::debug).format("new recv task: {} bytes", new_size);
			if (new_size == 0)
			{
